<template>
    <div>
        ...
        <review-window :isConfig="conFig" :commentId="commentId" @config="close" @colseUpdata="close"></review-window>
        <check-schedule v-if="conFig1" :isConfig="conFig1" :checkInfo="checkInfo" @close="close"></check-schedule>
    </div>
</template>
<script>
    ...
    export default {
        ...
        components: {reviewWindow, checkSchedule},
        data() {
            return {
                switchData: false,
                queryKey: "",//文章名称
                        ...
                        render: (h, params) => {
                            if (params.row.status == "0") {
                                return h('div', {style: {paddingLeft: 2}}, [
                                    h('Button', {
                                        props: {size: 'small'},
                                        style: {marginRight: "15px",display:this.btns['confirm-handle']?"block":"none"},
                                        on: {
                                            click: () => {
                                ...
                                return h('div', {}, [
                                    h('Button', {
                                        ...
                                        style: {marginRight: "15px",display:this.btns['confirm-delete']?"block":"none"},
                                        on: {
                                            click: () => {
        ...
        methods: {
            ...
            Query() {
                this.getCommentList(this.queryKey);
            },
            dealComment(rowId) {
